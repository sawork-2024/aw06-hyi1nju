...
let json_api = 'http://localhost:18081/';
...
$(document).ready(function () {
    $.fn.addToCart = function (id, count, stock) {
        ...
        if (stock == 1) {
            if (count > 0) {
                $.get(json_api + 'product_id' ,data={'productId':String(id)}, function (data) {
            ...
            $.get(json_api + 'product_id' ,{'productId':String(id)}, function (data) {
                $(this).addProductToCart(data);
            });
        }
    ...
    function loadProducts() {
        ...
        $.get(json_api + 'product', function (data) {
            console.log(data);
            data.forEach(item => {
                if (!(item.price instanceof String)){
    ...
    $.fn.submitDueOrder = function (status) {
        ...
        cart.forEach(item => {
            items_para.push(JSON.stringify({
                'product_id':item.id,
        ...
        $.get(json_api + 'submit_cart' ,{'items':JSON.stringify(items_para)}, function (data) {
        });
      ...
      <p style="text-align: center;">
       ${settings.footer}
        ...
        for (let i = 0; i < cart.length; i++) {
            console.log(cart[i]);
            $.ajax({
                url: json_api + 'product_id',
                dataType: "json",
                data: {
                    'productId': cart[i].id,
                    'quantity': cart[i].max_quantity - cart[i].quantity
                }
                ,
                type: "GET",
                // xhrFields: {
